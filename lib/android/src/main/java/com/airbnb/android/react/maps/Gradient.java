...
public class Gradient {
    ...
    public int mInitialColor = Color.TRANSPARENT;
     ...
     *                     This is given as an array of floats with values in the interval [0,1]
     * @param colorMapSize The size of the colorMap to be generated by the Gradient
     * @param initialColor The initial color of the gradient, that is used for 0 intensity
     */
    public Gradient(int[] colors, float[] startPoints, int colorMapSize, int initialColor) {
        if (colors.length != startPoints.length) {
            throw new IllegalArgumentException("colors and startPoints should be same length");
        } else if (colors.length == 0) {
            throw new IllegalArgumentException("No colors have been defined");
    ...
    private HashMap<Integer, ColorInterval> generateColorIntervals() {
        ...
        if (mStartPoints[0] != 0) {
            colorIntervals.put(0, new ColorInterval(mInitialColor, mColors[0], mColorMapSize * mStartPoints[0]));
        }
